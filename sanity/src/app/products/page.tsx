...
/**
 * Asynchronously fetches a list of products from the data source.
 *
 * @returns {Promise<Product[]>} A promise that resolves to an array of products.
 */
async function fetchProductList() {
  const products = await client.fetch('*[_type == "product"]'); // Added quotes around the query
  return products;
}
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden transform hover:scale-105 transition-transform duration-300 group">
      ...
      <div className="p-5">
        ...
        <div className="flex justify-between items-center">
          <p className="text-lg font-bold text-green-600">${Number(product.price).toFixed(2)}</p>
          <div className="text-yellow-500 flex items-center">
            <span className="text-sm font-medium mr-1">{product.rating.count}</span>
            <span>&#9733;</span>
          </div>
        </div>
        <Link href={`./products/${String(product.id)}`} className="block w-full bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white text-center py-3 mt-2 rounded-lg font-medium hover:scale-105 transition-transform duration-300">
          View Details
        </Link>
      </div>
    </div>
